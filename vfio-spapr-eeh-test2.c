...
void usage(char *name)
{
	...
	static inline uint64_t read_u32(struct device_t* dev, int offset) {
		__asm__ volatile("" : : : "memory");
    		return *((volatile uint64_t*)(dev->mmio_addr + offset));
	}
...
int main(int argc, char **argv)
{
	...
	sprintf(bdf_buf,"%04x:%02x:%02x.%d",seg, bus, dev, func);
	printf (" bdf_buf = %s - %d\n ", bdf_buf, strlen(bdf_buf));
	...
	for (i = 0; i < device_info.num_regions; i++) {
		...
		printf("size 0x%lx, offset 0x%lx, flags 0x%x , regions.flags 0b%b\n",
		...
		if( i==0) {
			offset = pcidev.regs[VFIO_PCI_BAR0_REGION_INDEX].offset+4;
		}
	}
	//pci read config                                                        
        pret = pread(device, &phys_val, 2,pcidev.regs[VFIO_PCI_CONFIG_REGION_INDEX].offset);                               
        if(pret < 0)                                                             
        {                                                                        
                perror("read onfig: ");                                          
        }                                                                        
	...
	pcidev.mmio_addr = mmap(NULL, bar_info->size, PROT_READ ,
...
*/
	...
	if(pret < 0) {
		perror("");
	eeh_dev_break_debugfs(bdf_buf, strlen(bdf_buf));
	}
	...
	* frozen.
	...
	//pci read config
	pret = pread(device, &phys_val, 2,pcidev.regs[7].offset);
	if(pret < 0)
	{
		perror("read onfig: ");
	}
